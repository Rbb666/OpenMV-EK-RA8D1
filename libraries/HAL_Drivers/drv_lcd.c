...
static void ra_bsp_lcd_set_display_buffer(uint8_t index)
{
    R_GLCDC_BufferChange(&g_display_ctrl, (uint8_t*)fb_background[index - 1], DISPLAY_FRAME_LAYER_1);
}

static void ra_bsp_lcd_set_working_buffer(uint8_t index)
{
    if (index >= 1 && index <= 2)
    {
...
static void ra_bsp_lcd_enable_double_buffer(void)
{
...
static void ra_bsp_lcd_swap_buffer(void)
{
...
static rt_err_t ra_bsp_lcd_init(void)
{
    ...
    /*  Display driver open */
    error = R_GLCDC_Open(&g_display_ctrl, &g_display_cfg);
...
static rt_err_t ra_lcd_control(rt_device_t device, int cmd, void *args)
{
    ...
    case RTGRAPHIC_CTRL_RECT_UPDATE:
    {
        ...
        g2d_display_write_area((uint8_t *)lcd->lcd_info.framebuffer, (uint8_t *)lcd_current_working_buffer,
                               info->width, info->height, info->x, info->y);

        /* swap lcd farmbuffer */
//        ra_bsp_lcd_swap_buffer();
...
int rt_hw_lcd_init(void)
{
    ...
    ra_lcd_init(device);
    ra_bsp_lcd_enable_double_buffer();
    ra_bsp_lcd_swap_buffer();
